...
const OtpScreen = ({ phoneNumber, generatedOtp, onVerifySuccess, onBack, onRegenerateOtp, isThemed = false }) => {
  ...
  return (
    <div className={`h-full w-full flex flex-col overflow-hidden ${isThemed ? 'bg-transparent' : 'bg-white'}`}>
      ...
      <div className={`flex-1 overflow-y-auto no-scrollbar min-h-0 ${isThemed ? 'pt-0 px-0' : 'px-5 pt-3'}`}>
        <h2 className={`text-[24px] font-black mb-1 leading-tight ${isThemed ? 'text-white' : 'text-black'}`}>Verify it's you</h2>
          ...
          {Array.from({ length: OTP_LENGTH }).map((_, i) => (
            <div key={i} className="flex-1">
              <input
                ...
                autoFocus={i === 0}
                className={`w-full h-16 text-center text-2xl font-black rounded-2xl outline-none transition-all duration-200 ${
                  isThemed 
